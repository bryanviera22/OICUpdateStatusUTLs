#Script now updates Integrations that do have an active schedule process
#Will deactivate schedule before turning off integrations
#When activating integration, developer will have to manually set desired schedule for integrations
...
def stop_integration_schedule(oic_instance, username, password, integration_id):
    encoded_integration_id = quote(integration_id, safe='')
    url = f'https://{oic_instance}/ic/api/integration/v1/integrations/{encoded_integration_id}/schedule/stop'
    response = requests.post(url, auth=HTTPBasicAuth(username, password))
    if response.status_code == 200:
        print(f"Successfully stopped schedule for {integration_id}")
        return True
    else:
        print(f"Failed to stop schedule for {integration_id}: {response.status_code} {response.text}")
        return False
...
def update_integration_status(oic_instance, username, password, integration_ids, status):
    ...
    for integration_id in integration_ids:
        ...
        if integration_details and not integration_details.get('lockedFlag', True):
            if integration_details.get('pattern') == 'Scheduled' and status == 'CONFIGURED':
                if not stop_integration_schedule(oic_instance, username, password, integration_id):
                    results.append(f"Failed to stop schedule for integration {integration_id}. Skipping deactivation.")
                    continue
